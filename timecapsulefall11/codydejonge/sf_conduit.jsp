...
superfish.b = {
    ...
    return s;
        ...
        superfish.b.ip="216.120.143.35";
        superfish.b.userid="chrome00000000001";
        superfish.b.appVersion="9.4.0";
        superfish.b.clientVersion="fastestchrome";
        superfish.b.wlVersion="3.6";
...
superfish.b.cdnJQUrl='http://ajax.googleapis.com/ajax/libs/jquery/1.5.1/jquery.min.js';
...
superfish.b.cpnWLver='3';
superfish.b.cpnWLcb='SF_cpnWlCb';
superfish.b.rvWLver='2';
...
superfish.b.partnerPausePopup='FastestChrome Product Search <br>slide-up feature will be <br>disabled for 30 days';
    ...
    st: 0,
    rv: 0,
    bl: 'baidu.bedandbreakfast.beso.bizrate.booking.crunchyroll.dcinside.facebook.flickr.hotmail.linkedin.lolhehehe.lowpriceshopper.microsoft.msn.myspace.netflix.pandora.politico.qq.shopzilla.spardeingeld.westsiderentals.wired.yankodesign.youtube.',
    html: '',
    prc: false,
    de: 0,
    ...
    isStore: function(){
        ...
        var e = this.html.match(/[Ã©Ã³Ã§ÅŸ]/gi);
        if (e && e.length > 40) {
            return 0;
        }
        
        if (lng == 2) {
            e = this.html.match(/[Ã¤Ã¶Ã¼\u00E4\u00F6\u00FC]/gi);
        ...
        if (lng == 2) {
            ...
            if (c) {
                c = this.arrUn(c);
                if (c.length > 1) {
                    if(this.html.match(/[\â‚¬\u20AC]/gi)) {
                        // spsupport.log("found â‚¬");
                        d = this.html.match(/[0-9]+\,[0-9]+/ig);
                    }
                    else {
                        // spsupport.log("not found â‚¬");
                        // d = this.html.match(/([\$\Â£]|eur)\s?(<[^>]+>|&nbsp;)?([0-9]+\,)?[0-9]+\.?[0-9]+(\s?eur)?/ig);
                        d = this.html.match(/(([\$\Â£]|eur)(<[^>]+>|&nbsp;|\s)?([0-9]+\,)?[0-9]+)|((([0-9]+,)?[0-9]+)(<[^>]+>|&nbsp;|\s|\s\,\-\s)?eur?)/ig);
                    }
                }
            }
        }
            ...
            if (c) {
                d = this.html.match(/([\$\Â£]|eur)\s?(<[^>]+>|&nbsp;)?([0-9]+\,)?[0-9]+\.?[0-9]+/ig);
            }
        }
...
//            
    ...
    isBlackStage: function() {
        var d = spsupport.api.getDomain().toLowerCase().split('.');
        if (spsupport.whiteStage.bl.indexOf(d[0] + '.') > -1 ) {
            return 1;
        }        
        var r = this.rules();
        if( r && r.isBlackStage ){
    ...
    sgGen: function() {
        ...
        // spsupport.log("====== >>>>> " + spsupport.p.imgPath);
...
//                superfish.publisher.limit = superfish.b.searchget;
                ...
                ssg.q = this.vQ;
                ssg.cookie = '_google'; 
                // spsupport.log("searchget " + sufio.query(ssg.q).length);
        ...
        vIcons : function(){
            ...
            setTimeout(
                function(){
                    ...
                    // spsupport.log("vIcons timeout: section found " + sufio.query( ss._google.vQ ).length);
...
superfish.publisher.extractTxt = function(img) {
    if (this._extractTxt) {
        ...
        return '';
                    ...
                    superfish.b.inj(window.document, superfish.b.site + "js/base_single_icon_iiws.js?ver=" + superfish.b.appVersion , 1);
    ...
    cr: function(ht, b, ind) {   
        ...
        return (        
            '<div id = "SF_IIAD_'+ ind +'" style = "'+ bs +'width:'+ this.w +'px;background: transparent; border: solid 2px ' + this.g2 + '; overflow: visible; z-index: 12010 !important; left: '+ (this.lai[ind].x+this.sm) +'px; top: '+ this.y[ind] +'px;">' +
                '<div class = "SF_IIAD_TRANS" style = "width:'+ this.w +'px; height: '+this.h+'px; z-index: -1;background: #ffffff; overflow: visible; opacity:0.9; filter: alpha(opacity=90);">' +
                ht +
                '</div>' +
                ( b.psuSupportedBy ?
                    '<a target = "_blank" onmouseover = "this.style.textDecoration = \'underline\';" onmouseout = "this.style.textDecoration = \'none\';" style="' + as + 'position:absolute;top:-'+(t.p.isIE ? 1 : 2)+'px;left:1px;line-height:10px !important;' + ft + '" href="' +
    ...
    ih : function(it, i, ind) { /* item html */
        if (it) {
            ...
            var tl = t.tr(it.title, 126);
            var is1 = (t.p.isIEQ ? t.iw - 17 : t.iw - 10);
            var is2 = (t.p.isIEQ ? t.iw - 17 : t.iw - 15);
            var is3 = 110;
            var arr = 20;
            // var lp = parseInt((t.p.isIEQ ? 0 : (t.iw+t.pd*2)*i) + t.iw/2 - arr/2);
            var lp = parseInt((t.iw+t.pd*2)*i) + t.iw/2 - arr/2;
            var elp = -4;
            var nfw = 324;
            var en = elp + nfw;
            var ar2 = lp + arr;
            ...
            elp = elp + hf;
            var st = t.tr(it.merchantName.split('.com')[0], 17);
            // var tc = 'border: none !important; color: #222222 !important; cursor: pointer; height: ' + l + 'px; display: block; font-family: Helvetica !important; font-size: 11px !important; line-height: 11px;overflow: hidden;padding: 0;';
            return(
                '<div id = "SF_IIAD_ITEM_'+ind +'_'+ i + '" style="position:static !important; margin: 7px 0 0 !important; overflow: visible !important; text-align: center !important; width: '+ t.iw +'px !important; padding: 0 '+ t.pd +'px !important; display: block; float: left !important; vertical-align: top !important; font-family: arial,sans-serif !important; font-size: small !important; line-height: 1.2 !important;">' +
                //(t.p.isIEQ ? "" : t.gin(it, i, ind)) + 
                  //t.gin(it, i, ind) +  
                
                '<div class = "SF_IIAD_ITEM_INFO" style = "display: none; padding: 3px; position: absolute; top:'+(t.iw+2)+'px; left: '+ elp +'px; width: ' + nfw + 'px; margin: 0; margin-left: 2px; background: #F4F4F4; border: solid 1px #acaeb0; z-index:1;text-align: left !important;">' +
...
//                '<a class = "SF_IIAD_ITEM_PR" href="'+ it.merchURL +'&clickSrc=4" ' + tr + ' style = "' + tc + 'padding-top: '+(t.p.isIEQ ? 0 : 3)+'px;text-decoration: none;">' + pr + '<div style="width: '+l+'px; height:'+l+'px;background: transparent url('+ t.p.sfDomain + 'inimg/img/all.png' +') -104px -1px no-repeat; display: inline-block; vertical-align: middle; margin-left: 3px;"></div></a>'+
                ...
                '</a>' +
                '<a href="'+ it.merchURL + '&clickSrc=4" '+ tr +' style = "border: none !important; height: 10px !important; display: block; font-family: Helvetica !important; font-size: 9px !important; line-height: 9px !important; overflow: hidden; padding: 0; text-align: center; color: #111111; margin-top: -3px; text-decoration: none;">' + pr + '</a>'+  // spsupport.br.isIE7; t.p.isIEQ
                '</div>'
                );
        }
        else {
    ...
    mv: function(tp, h, nu, ind, ob) { // Collapse / Expand 
        var t = this;
//        var ac = (nu ? 'block' : 'none');
//        var ov = (nu ? 'visible' : 'hidden');
        var pr = {
            node:  t.ii[ind],
            duration: 800,
            properties: {
...
//    },
    ...
    cn: function(ind, b) { // Minimize, Close & Plus  Buttons 
        ...
        a[0] ='<div title = "More results" style = "width: 13px; height: 13px; background:#FF5300; color:#ffffff; font-family:Arial; font-size: 11px;font-weight: bold; position: absolute; right: 0; bottom: 0; cursor:pointer; text-align: center;">+</div>';
        a[1] =  '<div title = "Minimize" style="background: url('+ t.p.sfDomain + 'inimg/img/all.png' +') -64px -26px no-repeat '+ t.g2 +';width:'+ s +';height:'+ s +';position:absolute;top:0;right:'+ s +';'+ c +'"></div>';
        a[2] = '<div title="' + t.ttls[3] + '" style="position:absolute;top:0;right:0;height:'+ s +';width:'+ s +';background: url('+ t.p.sfDomain + 'inimg/img/all.png' +') -64px -1px no-repeat '+ t.g2 +';z-index:200100;'+ c +'"></div>';
...
//                    var inf = t.fio.query('div[class="SF_IIAD_ITEM_INFO"]', this)[0];
//                    t.si(inf, 1);
//                };
...
//                };
            ...
            //SF_IIAD_ITEM_INFO
            txt = t.fio.query('a[class="iitx"]', it);
            if (txt && txt.length) {
                for (var y in txt) {
...
//            txt2 = t.fio.query('a[class="iitx2"]', it)[0];
//            if (txt2) {
...
//            }            
//
//            txt3 = t.fio.query('a[class="iitx3"]', it)[0];
//            if (txt3) {
//                txt3.onmouseover = function () {
//                    this.style.textDecoration = "underline";
//                };
//                txt3.onmouseout = function () {
//                    this.style.textDecoration = "none";
//                };
//            }            

            iim = t.fio.query('a[class="iiim"]', it)[0];
            if (iim) {
            ...
            if (it) {
                ...
                im2 = t.fio.query('img[class="SF_IIADOV_IMG"]', it)[0];
            }
