...
$(document).ready( function() {	
	...
	$("#logo a").css({"text-indent": "9999px", "background": "url('http://www.wallsave.com/images/wallsave.png') no-repeat"}).show();
	$("#result").hide();
	$(".keyword").watermark("Search Wallpapers");;
        
        //hide ticker
	$(".ticker-header i.close").click(function() {
	  $("#sidebarRight").animate({'width': '0%'}, 'slow');   	  
	  $("#thumbWrap").css({'margin': '0 0 0 204px'});	
	  $(".gradient2").animate({'opacity': '1'}, 'slow');	  
        });                 
		...
		$(this).css({"background": "url('http://www.wallsave.com/images/loading_gray.gif') repeat scroll 280px 50% white"});        
        });
		...
		//$('#menu>li>a').address();
	                ...
	                $('.sort').each(function() {
	                    if ($(this).attr('href') == ('http://www.wallsave.com/#!' + event.value)) {	                    
	                        ...
	                        $("span", this).css({"display": "inline"});
	                        $("#result").show();
				$("#result>div,#featured-front").fadeOut('10000', function(){$("#result>div,#featured-front").remove()});	                        
				
				var str = event.value;
				var clean = str.replace("/","");
				var fix = clean.replace("-"," ");
				var str = 'Best of ' +  fix + ' Wallpapers | Wallsave.com';       			               			
				...
				$.getJSON('best' + event.value, function(json){
					$.each(json.results,function(i,tab){	
						...
						$("#menu>li>a>span").hide();
						$("#thumbWrap").animate({'opacity': '1'}, 'slow');
						$(".gradient2").animate({'opacity': '1'}, 'slow');
						...
						$(".thumbs").hover(function(){
							$(".resInfo", this).animate({'opacity': '1'}, 'fast');
						},function(){
							$(".resInfo", this).animate({'opacity': '0'}, 'slow');
						}); 
					});
				});	       			
	                    } else {
	...
	$('.form').submit(function(e) {
		...
		var reso = $("input[name='reso']").val();
			...
			$("#container").animate({"padding-top" : "0", "width" : "95%"}, "slow");
			$("#reso-container").slideUp("fast");			
			$("#result>div,#featured-front,.category,#browse-result").fadeOut('5000', function(){$("#result>div,#featured-front,.category,#browse-result").remove()});	                        			
			
			$(".keyword").css({"background": "url('http://www.wallsave.com/images/loading_gray.gif') repeat scroll 280px 50% white"});
			$(".resolution").hide();
			$("#fullresult").hide();
			$(".sort").removeClass("selected");			
			$("#result").show();
				        
			$.post("/goleki/", {search: keyword, page: page, reso: reso}, 
				function(data){
					$("#result").html(data).css({"overflow": "hidden"});
	...
	$('.loadmore').live("click",function() {
		...
		$("#thumbWrap").animate({'margin': '0 0 0 204px'}, 'slow').delay(2000);
		$(".gradient2").animate({'opacity': '1'}, 'slow');					
				
		var ID = $(this).attr("id");
		if(ID){
			$(".loadmore").text('Painting...').css({"background": "url('http://www.wallsave.com/images/loading_gray.gif') repeat"});
			//$(".loadmore").hide().delay(2000);
			$("#sidebarRight").animate({'width': '0'}, 'fast');
	...
	$('.loadrelated').live("click",function() {
		...
		if(ID){	
			$(".loadrelated").text('Preparing...').css({"background": "url('http://www.wallsave.com/images/loading_gray.gif') repeat"});
